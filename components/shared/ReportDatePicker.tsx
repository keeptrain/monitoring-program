...
import { getIsfAvailableDatesByMonth } from "@/features/monitoring/actions/isf-dates";
...
export function ReportDatePicker({
  ...
  const { data: availableDates = [], isLoading } = useQuery({
    queryKey: ["isf-available-dates", zoneId, year, month],
    queryFn: () => getIsfAvailableDatesByMonth(zoneId, year, month),
    enabled: isOpen,
    staleTime: 5 * 60 * 1000, // 5 menit cache agar tidak fetch berulang saat ganti bulan maju-mundur
  });
  ...
  const disabledDays = (date: Date) => {
    // Set to 0:0:0 to match correctly
